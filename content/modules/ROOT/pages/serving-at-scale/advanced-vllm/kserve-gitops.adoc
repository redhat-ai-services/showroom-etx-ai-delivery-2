...
[source,bash,role="execute"]
----
helm upgrade -i granite-guardian -n vllm redhat-ai-services/vllm-kserve --version 0.5.9 \
  --set fullnameOverride="granite-guardian" \
  --set image.tag="rhoai-2.22-cuda-5e9c649953464aa3a668aba1774e42fc933f721b" \
...
[source,bash,role="execute"]
----
helm upgrade -i granite-8b -n vllm redhat-ai-services/vllm-kserve --version 0.5.9 \
...
====
```
helm upgrade -i vllm-multi-node-llama -n vllm redhat-ai-services/vllm-kserve --version 0.5.9 \
